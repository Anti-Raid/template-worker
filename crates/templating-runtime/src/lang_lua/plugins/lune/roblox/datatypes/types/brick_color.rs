...
impl<'lua> LuaExportsTable<'lua> for BrickColor {
    ...
    fn create_exports_table(lua: &'lua Lua) -> LuaResult<LuaTable> {
        ...
        let mut builder = TableBuilder::new(lua)?
            ...
            .with_function("palette", |_, index: u16| {
                if index == 0 {
        ...
        for (name, number) in BRICK_COLOR_CONSTRUCTORS {
            builder = builder.with_function(*name, |_, ()| Ok(color_from_number(*number)))?;
        }
