...
impl<'lua> LuaExportsTable<'lua> for CFrame {
    ...
    fn create_exports_table(lua: &'lua Lua) -> LuaResult<LuaTable> {
        ...
        TableBuilder::new(lua)?
            .with_function("Angles", |_, (rx, ry, rz): (f32, f32, f32)| {
                Ok(CFrame(Mat4::from_euler(EulerRot::XYZ, rx, ry, rz)))
            })?
            .with_value("identity", CFrame(Mat4::IDENTITY))?
            .with_function(
                "fromAxisAngle",
                |_, (v, r): (LuaUserDataRef<Vector3>, f32)| {
                    Ok(CFrame(Mat4::from_axis_angle(v.0, r)))
                },
            )?
            .with_function("fromEulerAnglesXYZ", |_, (rx, ry, rz): (f32, f32, f32)| {
                Ok(CFrame(Mat4::from_euler(EulerRot::XYZ, rx, ry, rz)))
            })?
            .with_function("fromEulerAnglesYXZ", |_, (rx, ry, rz): (f32, f32, f32)| {
            ...
            .with_function("fromOrientation", |_, (rx, ry, rz): (f32, f32, f32)| {
                Ok(CFrame(Mat4::from_euler(EulerRot::YXZ, ry, rx, rz)))
            })?
            .with_function(
                "lookAt",
            ...
            .with_function("new", |lua, args: LuaMultiValue| match args.len() {
