    ...
    async fn validate_error_channel(
        ...
        match self.id {
            Id::GuildId(guild_id) => {
                ...
                if guild_channel.base.guild_id != guild_id {
                    return Err(
...
impl DataStoreImpl for TemplateStore {
    ...
    fn get_method(&self, key: String) -> Option<DataStoreMethod> {
        match key.as_str() {
            "list" => {
                let state_ref: WorkerState = self.state.clone(); // reference to the state data
                let id = self.id;
                Some(DataStoreMethod::Async(Rc::new(move |_v| {
                    match id {
                        Id::GuildId(guild_id) => {
                            let state = state_ref.clone(); // satisfy rusts borrowing rules
                            Box::pin(async move {
            ...
            "get" => {
                let state_ref = self.state.clone(); // reference to the guild state data
                let id = self.id;
